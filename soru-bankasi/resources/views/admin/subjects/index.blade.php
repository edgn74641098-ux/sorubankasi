@extends('layouts.admin', ['pageTitle' => 'Ders Yönetimi', 'title' => 'Ders Yönetimi'])

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">Dersler</h1>
            <p class="text-muted mb-0">Dersleri yönetin, aktiflik durumlarını takip edin.</p>
        </div>
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.subjects.index') }}" class="row g-3 mb-4">
                ...
                <div class="col-md-8 d-flex align-items-end gap-2">
                    ...
                    <a href="{{ route('admin.subjects.index') }}" class="btn btn-outline-secondary">Sıfırla</a>
                </div>
            </form>

            @if($subjects->isEmpty())
                <div class="alert alert-info mb-0">Kayıtlı ders bulunamadı.</div>
            @else
                        ...
                        <tbody>
                            @foreach($subjects as $subject)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('admin.subjects.edit', $subject) }}" class="btn btn-sm btn-outline-primary">Düzenle</a>

                                        @can('delete', $subject)
                                            <form method="POST" action="{{ route('admin.subjects.destroy', $subject) }}" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Bu dersi pasif yapmak istediğinize emin misiniz?')">Pasif Yap</button>
                                            </form>
                                        @endcan
                                    </td>
                                </tr>
